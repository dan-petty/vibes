...
| [**Adaptive Web Crawler**](./examples/adaptive-web-crawler/) | Two-tier adaptive crawler detecting SPA shells, escalating to headless browsing, persisting learned domain strategies, and emitting Markdown shaped for a prompt — fenced code, resolved links, escaped page text. | `pytest examples/adaptive-web-crawler/` |
| [**Prompt Mutation Fuzzer**](./examples/prompt-mutation-fuzzer/) | Grammar-guided prompt perturbation and multi-provider model driver (Mock, OpenAI, Anthropic, Ollama, REST) evaluating agent code invariant resilience. | `pytest examples/prompt-mutation-fuzzer/test_fuzzer.py` |
...
- [**Semantic Graph AST Code Memory & Symbol Indexer (`tools/code_memory.py`)**](./tools/code_memory.py): Persistent AST symbol index and directed call graph memory maintaining def-use chains, signature stability (`SGM002`), circular dependency avoidance (`SGM003`), and transitive blast radius analysis across multi-file refactoring sessions with OASIS SARIF 2.1.0 exports and WCAG AA Mermaid visualizations.
...
│   ├── contract_variables.py          # Shared: contract questions, answers and safe rendering
...
└── tests/                             # Automated test suites for tools and harnesses
    ...
    ├── test_living_standard_certifier.py # Unit tests for living standard certifier & attestation
    ...
    ├── test_finding_baseline.py       # Unit tests for baseline acceptance, staleness and pruning
