...
- [x] **Fast-Feedback Test Optimization & Isolated Runner**:
  ...
  - Phase 4 (Planned): Amortize per-resource interpreter boot by executing the repository sweep through a single warm pytest session with per-file result attribution, reclaiming the ~1.7s x N seconds currently spent re-initialising the interpreter on every scanned resource.
...
| **Suppression Ergonomics: Waivers That Cannot Become Mute Buttons** | `patterns/` | Which invariants may a codebase ever waive, and what makes a waiver auditable rather than an escape hatch that erodes the gate? | 🔬 In Queue |
| **Silent Certification Failures in Mechanical Gates** | `observations/methodology/` | When an enforcement tool reads only the first of N inputs and reports success, how long does a green checkmark conceal unaudited code? | 🔬 In Queue |
| **Oracle Measurement Validity & Harness-Smeared Metrics** | `observations/methodology/` | When a deterministic oracle measures its own scaffolding, how long does an agent chase an unfixable defect before suspecting the ruler rather than the object? | 🔬 In Queue |
