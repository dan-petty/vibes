...
- [x] **Fast-Feedback Test Optimization & Isolated Runner**:
  ...
  - Phase 5 (Planned): **Directory Map Staleness Oracle** — a `docs_validator` rule diffing the `text` directory trees embedded in `README.md` against the filesystem. Three index counts (32 / 25 / 21 case studies against 33 actual) and three subtrees (observations and patterns, frozen at 15 / 7 / 13 entries) drifted undetected because a text tree breaks no links and fails no test. Prose is the one artifact nothing executes; give it an oracle.
