...
The fundamental insight across all 39 observations is simple yet profound:
...
| [**17**](./systems/17-a-fuzzers-first-report-is-about-the-fuzzer.md) | `systems` | [A Fuzzer's First Report Is About the Fuzzer](./systems/17-a-fuzzers-first-report-is-about-the-fuzzer.md) | Three consecutive first runs of one new fuzzer returned 0 findings across 320 cases (two of eight instruments were never reached — the roadmap parser reads a grammar, not prose), then 10 cross-seed divergences of which 10 were false (the harness's own scratch directory was inside the comparison), then 1 real defect. | Count reach before believing a clean run; normalize whatever the harness introduced out of every two-execution comparison; drive each property from a deliberately broken target; separate the random search from the deterministic corpus that gates. | Instruments reached 6/8 to 8/8; roadmap findings 0 to 86 over 30 cases; cross-seed divergences 10 reported/0 real to 0/0; 1 latent defect found at 4,000 cases and minimized to 4 lines. |
